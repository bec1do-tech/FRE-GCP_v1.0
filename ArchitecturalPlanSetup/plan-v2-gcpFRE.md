# GCP Account Setup Plan — FRE_GCP_v1.0 (v2)

> **v2 changes vs v1:** Added VAIS (Vertex AI Search / Gemini Enterprise) and Web Context
> (Google Search) as search backends; added parallel signed-URL injection flow in diagrams 1 & 3.
> Diagram 2 (Ingestion Pipeline) is unchanged.
...
** Recommended by Joshua / now Lead / ex-Solution Architect:.** 
USE agentcard.json as the following (change it accroding to our settings when we are at deployment stage please):
...
```json{
  ...
  "description": "Test Report Search Engine - finding and compiling information from internal test reports",
  "name": "agent_trse",
  "skills": [
    {
      "id": "test_report_search",
      "name": "Test Report Search Engine",
      "description": "Find and compile information from internal test reports",
      "tags": ["test", "report", "search", "engine"]
    }
  ],
  "url": "https://adk-trse-poc-v2-978308790141.europe-west4.run.app/a2a/agent_trse",
  "version": "0.2.0"
}
...
### 1. Overall System Architecture *(updated in v2)*
...
graph TB
    ...
    subgraph GCP["☁️ Google Cloud Platform — us-central1"]
        ...
        subgraph AI["AI / ML"]
            ...
            VAIS["Vertex AI Search<br>Gemini Enterprise<br>(FileResearchEngine app)"]
        end
    end
    ...
    subgraph WEB["🌐 External"]
        GSEARCH["Google Search<br>(web_context_agent)"]
    end
    ...
    CR -->|"Managed RAG search"| VAIS
    CR -->|"Live web context"| GSEARCH
    CR -->|"Read / write sessions"| CSQL
    CR -->|"Sign doc URLs<br>(IAM Credentials API)"| GCS
    GCS -->|"Signed HTTPS URL"| CR
    CR -->|"Clickable doc links"| U
...
### 2. Document Ingestion Pipeline *(unchanged from v1)*
...
### 3. Query / Search Flow *(updated in v2)*
...
sequenceDiagram
    ...
    participant VAIS as Vertex AI Search<br/>Gemini Enterprise
    participant GSEARCH as Google Search<br/>Web Context
    participant GCS as Cloud Storage<br/>IAM Signing
    participant CSQL as Cloud SQL<br/>PostgreSQL

    User->>CR: Natural language query
    CR->>GEM: Route query to search pipeline

    par Parallel Search (ParallelAgent — 4 agents)
        GEM->>ES: BM25 keyword search<br/>es_search_agent
        ES-->>GEM: Top-K keyword hits + http_url per result
    and
        GEM->>VAI: Semantic vector search<br/>vertex_search_agent
        VAI-->>GEM: Top-K semantic hits + http_url per result
    and
        GEM->>VAIS: Managed RAG search<br/>vais_search_agent
        VAIS-->>GEM: Grounded excerpts + http_url per result
    and
        GEM->>GSEARCH: Live web context<br/>web_context_agent
        GSEARCH-->>GEM: External sources & citations
    end

    Note over ES,VAIS: Each search tool calls _batch_sign_uris()<br/>parallel ThreadPoolExecutor signs all GCS URIs<br/>→ injects http_url into every result dict

    GEM->>GEM: Reciprocal Rank Fusion<br/>score = Σ 1/(k+rank), k=60
    GEM->>GEM: Synthesize cited answer<br/>synthesis_agent<br/>Sources Consulted = links from search context
    GEM-->>CR: Final answer + [filename](https://signed-url) links
    CR->>CSQL: Persist conversation turn
    CR-->>User: Answer with clickable document links
```
